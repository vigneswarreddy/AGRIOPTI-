...
function Fertilizers() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const q = query(collection(db, "products"), where("productType", "==", "fertilizers"));
        ...
        console.error("Error fetching fertilizers from Firebase:", error);
      }
    };
    fetchProducts();
  }, []);
        ...
        <div className="flex items-end justify-between mb-12">
          <div className="space-y-1">
            <h2 className="text-3xl md:text-4xl font-heading font-black text-nature-950 tracking-tight">
              Soil <span className="text-nature-600">Nutrition</span>
            </h2>
            <p className="text-nature-500 font-bold uppercase tracking-widest text-xs">Premium Fertilizers</p>
          </div>
          <button
            onClick={() => navigate('/market/fertilizers')}
