...
function Machinery() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const q = query(collection(db, "products"), where("productType", "==", "machinery"));
        ...
        console.error("Error fetching machinery from Firebase:", error);
      }
    };
    fetchProducts();
  }, []);
  ...
  return (
    <div className="w-full py-16 px-6 md:px-12 lg:px-20 bg-nature-50/30">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-12">
          <div className="space-y-1">
            <h2 className="text-3xl md:text-4xl font-heading font-black text-nature-950 tracking-tight">
              Heavy <span className="text-nature-600">Machinery</span>
            </h2>
            <p className="text-nature-500 font-bold uppercase tracking-widest text-xs">High-performance equipment</p>
          </div>
          <button
            onClick={() => navigate('/market/machinery')}
