...
function Crops() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const q = query(collection(db, "products"), where("productType", "==", "crops"));
        ...
        <div className="flex items-end justify-between mb-12">
          <div className="space-y-1">
            <h2 className="text-3xl md:text-4xl font-heading font-black text-nature-950 tracking-tight">
              Fresh <span className="text-nature-600">Crops</span>
            </h2>
            <p className="text-nature-500 font-bold uppercase tracking-widest text-xs">Direct from the farm</p>
          </div>
          <button
            onClick={() => navigate('/market/crops')}
